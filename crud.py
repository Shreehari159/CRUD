...
def insert():
    ...
    res = con.cursor()
    sql = "Insert into data (name, age, address, contact, gmail) values (%s,%s,%s,%s,%s)"
...
def update():
    ...
    print("4.Contact")
    print("5.Mail")
    option = int(input("/nWhich field you want to update?"))
    if option == 1:
        ...
        cur = con.cursor()
        sql = "UPDATE data set gmail=%s where pid=%s"
        cur.execute(sql, (gmail, pid))
        con.commit()
        select()
        print("/n")
        print("Update Successfully")
